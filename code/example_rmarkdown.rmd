...
subtitle: Introduction to R Course
author: |
...
This is an example of a R Markdown document. With some basic formatting codes you can make text *italic* and **bold**.
...
[comment]: This is a comment in Rmarkdown.
...
Actually, we do not typically show R output. Instead, we create graphs and tables based on the results of our analysis and include these in the document. Using regular R code, you can create graphs, which then get automatically embedded in the document.

```{r models, fig.width=7, fig.height=5.5, fig.align='center', fig.cap='R Skills as a Function of Time in the Course', echo=FALSE}
par(mar=c(5,4,2,2))
...
```{r quadmod, echo=FALSE, results='asis'}
kable(round(coef(summary(res2)), 3), caption="Results from Quadratic Model")
```

See Table \@ref(tab:quadmod) for the results for the quadratic model.
...
```{r, echo=FALSE, results='asis'}
knitreg(list(res1, res2), caption="Comparison of the Linear and Quadratic Models")
```
